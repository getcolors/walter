...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='colors-ssh-ansible-') as directory:
        ...
        legacy = '# BEGIN walter probe ANSIBLE MANAGED BLOCK\nHost probe\n    HostName 192.0.2.1\n    User obsolete\n# END walter probe ANSIBLE MANAGED BLOCK\n'
        retired = '# BEGIN walter probe-retired ANSIBLE MANAGED BLOCK\nHost probe-retired\n    HostName 192.0.2.2\n    User retired\n# END walter probe-retired ANSIBLE MANAGED BLOCK\n'
        config.write_text(legacy + retired + unrelated)
